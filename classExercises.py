...
class Conta_corrente:
    def __init__(self):
        self.nome_correntista = input('')
        self.servicos_extras = eval(input(''))
        self.saldo_conta = float(input(''))
...
for conta in listaDeContas:
    if conta.saldo_conta < 100:
        print(conta)
        conta.saldo_conta +=200

for conta in listaDeContas:
    if conta.servicos_extras:
        conta.saldo_conta -=50
    if not conta.servicos_extras:
        conta.saldo_conta -=10

for conta in listaDeContas:
    if conta.saldo_conta > 1000:
        listaDeContas.remove(conta)
    
newlist = sorted(listaDeContas, key = Conta_corrente.get_saldo_conta, reverse = True)
